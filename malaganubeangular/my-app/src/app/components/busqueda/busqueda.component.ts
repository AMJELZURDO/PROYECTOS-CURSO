...
export class BusquedaComponent implements OnInit {
  ...
  restaurateTocado (restaurante:Restaurante)
  {
    ...
    this.mapaHijo.dibujarPunto(restaurante.latitud, restaurante.longuitud, restaurante.nombre);
  }
  ...
  encuentrame(){
    if (navigator.geolocation){
      console.log("tenemos acceso al API geolocalication");
      navigator.geolocation.getCurrentPosition((pos)=>this.exito(pos),
      ()=>this.fracaso());
    }else{
      console.log("NO tenemos acceso al API geolocalication");
    }
